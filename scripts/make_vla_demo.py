"""Render the zero-shot VLA playing (and failing) one episode per class.

Same 4-view + HUD layout as make_demo.py; collisions get a red COLLISION flash.

    python scripts/make_vla_demo.py          # needs a transformers env (habvln)
"""
...
def main() -> None:
    ...
    cfg = load_config()
    gen = SceneGenerator(cfg, seed=7)          # same levels as the oracle demo
    env = PassageEnv(cfg)
    ...
    for label in ("feasible", "rotation_required", "impossible"):
        ...
        frames.append(compose(env, f"VLA·{label}", "START", 0, 0.0))
        for _ in range(30):
            act = agent.act(obs["rgb"])
            obs, r, term, trunc, info = env.step(act)
            f = compose(env, f"VLA·{label}", ACTIONS[act], info["step"],
    ...
    out = RESULTS / "01_demos" / "02_vla_zeroshot_three_classes.gif"
