...
## [0.5.0] - 2026-07-19
### Features(S3 多视角消融:感知墙归因验证)
- 采集支持 `--multi`(main+front+side 9通道@224²,train_mv.npz 860MB);PassageCNN `in_ch` 参数;
  训练 uint8 按 batch 归一化(避免 26GB float 峰值);eval 按权重 in_ch 自动组帧。
### Notes & Caveats
- 闭环阶梯(同 30 关):单视角 feasible 5/10, rotation 1/10 → **+正/侧视图 8/10, 1/10** →
  **+本体感知 7/10, 3/10**;认输三者皆 10/10 零误报。
- 归因坐实:正视图补齐 dx/dz → 平移对齐基本修复;旋转失败模式从"不转/无限转"变为
  "转到大致正确步数后差几度擦碰",加 proprio(当前θ免费)后 1/10→3/10。剩余缺口=
  目标θ与孔宽的 2.5° 级精确匹配,需更多数据或显式θ回归。
- 结论:控制瓶颈确为感知(输入信息),非算法/容量——与"单视角回归天花板 6cm/10°"诊断互证。
